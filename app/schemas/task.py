...
class TaskBase(BaseModel):
    ...
    description_md: Optional[str] = Field(None, description="태스크 설명 마크다운")
    type: str = Field(
        default="feat",
        description="태스크 타입",
    ...
    source: str = Field(
        default="USER",
        description="태스크 생성 소스",
        examples=["MCP", "USER", "AI"]
    )
    status: str = Field(
        default="todo",
        description="태스크 상태",
...
class TaskUpdate(BaseModel):
    ...
    description: Optional[str] = Field(None, description="태스크 설명")
    description_md: Optional[str] = Field(None, description="태스크 설명 마크다운")
    type: Optional[str] = Field(None, description="태스크 타입")
    status: Optional[str] = Field(None, description="태스크 상태")
    priority: Optional[int] = Field(None, description="우선순위 (0-10)", ge=0, le=10)
    tags: Optional[List[str]] = Field(None, description="태그 목록")
    due_at: Optional[datetime] = Field(None, description="마감일")
    result_files: Optional[List[str]] = Field(None, description="생성/수정된 파일 목록")
    summary: Optional[str] = Field(None, description="작업 요약")
    duration: Optional[int] = Field(None, description="작업 소요 시간 (초 단위)", ge=0)
    result_logs: Optional[str] = Field(None, description="결과 로그 (마크다운 형식)")
...
class TaskResponse(TaskBase):
    ...
    result_files: Optional[List[str]] = Field(None, description="생성/수정된 파일 목록")
    summary: Optional[str] = Field(None, description="작업 요약")
    duration: Optional[int] = Field(None, description="작업 소요 시간 (초 단위)")
    result_logs: Optional[str] = Field(None, description="결과 로그 (마크다운 형식)")
    created_at: datetime
    updated_at: datetime
