...
def upgrade() -> None:
    # tasks 테이블에 새 필드 추가
    op.add_column('tasks', sa.Column('description_md', sa.CLOB(), nullable=True, comment='태스크 설명 마크다운 (CLOB)'))
    op.add_column('tasks', sa.Column('type', sa.String(length=50), nullable=False, server_default=sa.text("'feat'"), comment='태스크 타입'))
    op.add_column('tasks', sa.Column('source', sa.String(length=50), nullable=False, server_default=sa.text("'USER'"), comment='태스크 생성 소스'))
    op.add_column('tasks', sa.Column('tags', sa.CLOB(), nullable=True, comment='태그 목록 (JSON 배열 문자열)'))
    op.add_column('tasks', sa.Column('due_at', sa.DateTime(timezone=True), nullable=True, comment='마감일'))
    op.add_column('tasks', sa.Column('result_files', sa.CLOB(), nullable=True, comment='생성/수정된 파일 목록 (JSON 배열 문자열)'))
    op.add_column('tasks', sa.Column('summary', sa.CLOB(), nullable=True, comment='작업 요약'))
    op.add_column('tasks', sa.Column('duration', sa.Integer(), nullable=True, comment='작업 소요 시간 (초 단위)'))
    op.add_column('tasks', sa.Column('result_logs', sa.CLOB(), nullable=True, comment='결과 로그 (마크다운 형식)'))
